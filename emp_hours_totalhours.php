...
$query_period = "SELECT * FROM tbl_settings";
$period = mysql_query($query_period, $security) or die(mysql_error());
$row_period = mysql_fetch_assoc($period);
$totalRows_period = mysql_num_rows($period);


mysql_select_db($database_security, $security);
$query_hours = "SELECT hou.*, loc.location FROM tbl_hours hou, tbl_users us, locations loc WHERE hou.loc_id = loc.loc_id AND hou.users_id= us.users_id AND SUBSTRING(shift_start,1,10) >= '" . $row_period['date_start']. "' AND SUBSTRING(shift_start,1,10) <= '" . $row_period['date_end']. "' AND us.username = \"" . $_SESSION['MM_Username'] . "\" ORDER BY shift_start DESC";
...
<table width="600" border="0" align="center">
  <tr>
    <td height="466" align="center" valign="top"><!-- InstanceBeginEditable name="content" -->
      <p>&nbsp;</p>
      <p class="Titles">Total hours worked from <?php echo $row_period['date_start']; ?> to <?php echo $row_period['date_end']; ?></p>
      <form id="form1" name="form1" method="post" action="">
        <input name="hidden_start" type="hidden" id="hidden_start" value="<?php echo $row_period['date_start']; ?>" />
        <input name="hidden_end" type="hidden" id="hidden_end" value="<?php echo $row_period['date_end']; ?>" />
      </form>
      <br />
      <table width="684" border="1">
        <tr class="cabecera">
          <td width="159">Shift started</td>
          <td width="142">Shift ended</td>
          <td width="85" align="center">Hours</td>
          <td width="160">Location</td>
          <td width="76">Comments</td>
        </tr>
        <?php do { ?>
          <tr>
          ...
          <?php
		   $total_hours =  $total_hours + $row_hours['hours'];
		  } while ($row_hours = mysql_fetch_assoc($hours)); ?>
</table>
      <p>Total hours = <?php echo "<font size=\"+3\">" . $total_hours; ?></font></p>
      <p>&nbsp;</p>
      <p>&nbsp;</p>
