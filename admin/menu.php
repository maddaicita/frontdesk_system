...
mysql_select_db($database_security, $security);
$query_task = sprintf("SELECT task.*, us.first_name, us.last_name, loc.location FROM tbl_admins adm, tbl_pending_task task, tbl_users us, locations loc WHERE task.loc_id= loc.loc_id AND adm.username = %s AND task.admin_id = adm.users_id AND task_status=1 AND task.users_id = us.users_id", GetSQLValueString($colname_task, "text"));
  ...
  <tr>
    <td height="568" align="center" valign="top"><!-- InstanceBeginEditable name="Content" -->
      ...
      <?php if ($totalRows_task > 0) { // Show if recordset not empty ?>
        <p class="Titles">Pending task list for <?php echo $row_user['first_name']; ?> <?php echo $row_user['last_name']; ?></p>
        <table width="600" border="1">
          ...
          <?php do { ?>
            <tr>
              ...
              <td><?php echo $row_task['last_name']; ?>, <?php echo $row_task['first_name']; ?></td>
              <td><?php echo $row_task['location']; ?></td>
              <td align="center"><a href="task_details.php?task=<?php echo $row_task['task_id']; ?>"><img src="../images/detalle.png" width="16" height="16" /></a></td>
              <td align="center"><img src="../images/delete.png" width="16" height="16" /></td>
            </tr>
            <?php } while ($row_task = mysql_fetch_assoc($task)); ?>
        </table>
        <?php } // Show if recordset not empty ?>
      <?php if ($totalRows_task == 0) { // Show if recordset empty ?>
  <table width="400" border="0">
    <tr>
      <td align="center">There is no pending task on your scheduler</td>
      </tr>
  </table>
