...
$check_in_full = $row_schedule['sch_date'] . " " . $check_in . ":00";
	...
	$check_out_full = $row_schedule['sch_date'] . " " . $check_out . ":00";
		...
		$check_out_full = strtotime(date("Y-m-d", strtotime($row_schedule['sch_date'])) . " +1 day");
		$check_out_full =  date("Y-m-d",$check_out_full) .  " " . $check_out . ":00";
 ...
 $Result1 = mysql_query($updateSQL, $security) or die(mysql_error());
	...
	//IF A RECORD HAS BEEN FOUND UPDATE THE FINGERPRINT TABLE WITH 2 AT SCHE_ID and tipe record
