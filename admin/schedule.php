...
$colname_locat = "-1";
if (isset($_GET['loc'])) {
  $colname_locat = $_GET['loc'];
}
mysql_select_db($database_security, $security);
$query_locat = sprintf("SELECT * FROM locations WHERE loc_id = %s", GetSQLValueString($colname_locat, "int"));
$locat = mysql_query($query_locat, $security) or die(mysql_error());
$row_locat = mysql_fetch_assoc($locat);
$totalRows_locat = mysql_num_rows($locat);
?>
...
<table width="800" border="0" align="center">
  ...
  <tr>
    <td height="568" align="center" valign="top"><!-- InstanceBeginEditable name="Content" -->
      <p class="Titles">Schedule by property - <?php echo $row_locat['location']; ?></p>
      <table width="800">
  <tr align="center">
...
echo "<br>" . $row_dates['shift_time'];
	...
	//bring the data base records for scheduled timeshifts
	mysql_select_db($database_security, $security);
	$query_dates = "SELECT sch.shift, us.first_name, us.last_name FROM schedule sch, locations_shifts sft, locations loc, tbl_users us WHERE sch.users_id = us.users_id AND sft.loc_id = loc.loc_id AND sft.shift_id=sch.shift_id AND sch.sch_date = '" . date("Y-m-d", strtotime($cYear . "-" . $cMonth . "-" . ($i - $startday + 1))) . "' AND loc.loc_id = " .$loc . " ORDER BY sch.shift, us.first_name ASC";
	//echo $query_dates;
	$dates = mysql_query($query_dates, $security) or die(mysql_error());
	$row_dates = mysql_fetch_assoc($dates);
	$totalRows_dates = mysql_num_rows($dates);	
...
do { 
echo "<p class=\"lineas\" >" . $row_dates['first_name']. " " . $row_dates['last_name'];
echo "<br $query_employees>" .$row_dates['shift'];
} while ($row_dates = mysql_fetch_assoc($dates));
