...
mysql_select_db($database_security, $security);
$query_hours = "SELECT usu.last_name, usu.middle_name, usu.first_name, usu.pay_rate, SUM(hours) FROM tbl_hours hou, tbl_users usu WHERE hou.users_id=usu.users_id AND hou.shift_start >='" . $_REQUEST['date_start'] . "' AND hou.shift_end <='" . $_REQUEST['date_end'] . "' GROUP BY hou.users_id ORDER BY usu.first_name";
...
<table width="800" border="0" align="center">
  ...
  <tr>
    <td height="568" align="center" valign="top"><!-- InstanceBeginEditable name="Content" -->
      <p class="Titles">General hours list<br />
      From: <?php echo $_REQUEST['date_start']; ?> to: <?php echo $_REQUEST['date_end']; ?> </p>
      <?php if ($totalRows_hours > 0) { // Show if recordset not empty ?>
  <table width="469" border="1">
    <tr class="cabecera">
      <td width="182">Names</td>
      <td width="101">Total hours</td>
      <td width="62">Rate x hour</td>
      <td width="96">Total to pay</td>
    </tr>
    <?php do { ?>
      <tr>
        <td><?php echo $row_hours['first_name']; ?> <?php echo $row_hours['middle_name']; ?> <?php echo $row_hours['last_name']; ?></td>
        <td align="center"><?php echo $row_hours['SUM(hours)']; ?></td>
        <td><?php echo $row_hours['pay_rate']; ?></td>
        <td><?php echo $row_hours['SUM(hours)'] * $row_hours['pay_rate'];  ?></td>
      </tr>
