...
if ((isset($_GET['fix_id'])) && ($_GET['fix_id'] != "")) {
  $deleteSQL = sprintf("DELETE FROM tbl_finger_fix WHERE fix_id=%s",
                       GetSQLValueString($_GET['fix_id'], "int"));

  mysql_select_db($database_security, $security);
  $Result1 = mysql_query($deleteSQL, $security) or die(mysql_error());

  $deleteGoTo = "employee_hours_list.php?id=" . $_REQUEST['id'] . "";
