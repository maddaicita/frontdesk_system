...
mysql_select_db($database_dplace, $dplace);
if (($_POST['names'] == "") or ($_POST['names'] == " ")) { 
	$query_tennants = "SELECT * FROM tbl_tennants WHERE `phones`=11111111";
} else {
	$query_tennants = sprintf("SELECT * FROM tbl_tennants WHERE `phones` LIKE %s", GetSQLValueString("%" . $colname_tennants . "%", "text"));
}
...
<table width="692" height="367" border="0" align="center">
  <tr>
    <td align="center" valign="top"><form id="form1" name="form1" method="post" action="search_phone.php">
      <br />
      Please introduce a part or the whole telephone number.<br />
      <br />
      <table width="469" border="1">
        <tr>
          <td width="126" height="62" bgcolor="#333333" class="etiqueta">Phone number to search:</td>
          <td width="240"><label>
      ...
      <table width="752" border="1">
        <?php if ($totalRows_tennants > 0) { // Show if recordset not empty ?>
          <tr bgcolor="#333333" class="etiqueta">
            <td width="269">Tennant</td>
            <td width="354">Phone number</td>
            <td width="107">Details</td>
          </tr>
          <?php do { ?>
            <tr>
              <td height="35" align="left" valign="middle" class="linea"><?php echo $row_tennants['names']; ?></td>
<td align="left" class="linea"><?php
			  $keywords = $_POST['names'];
			  $str = $row_tennants['phones']; 
